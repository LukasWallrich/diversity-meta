...
available <- list.files("../SM1 - Search and deduplication/full_text", full.names = TRUE, recursive = TRUE, pattern = ".pdf") 

pdfs$path <- available[map_int(pdfs$file_name, \(f) str_detect(available, f) %>% which() %>% c(NA) %>% .[1])]

missing_pdfs <- pdfs %>% filter(is.na(path)) %>% left_join(dataset %>% select(id, file), by = "id", multiple = "first") 

walk(pdfs %>% filter(!is.na(path)) %>% pull(path), 
     ~file.rename(.x, file.path("../SM1 - Search and deduplication/full_text/included", basename(.x))))
