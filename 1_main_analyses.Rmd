...
```{r}
single_moderator_tests <- cat_moderators %>% set_names() %>% map(\(moderator) {
  message("Now estimating ", moderator)
  ...
  mod <- with(imp, {
    ...
    assign(moderator, factor(get(moderator), ordered = FALSE))
    ...
    list(
      mod = mod,
      mod_interact = mod_domains,
      robu_main = coef_test(mod, vcov = "CR2"),
      robu_interact = coef_test(mod_domains, vcov = "CR2")
    )
  })

  # Pool coef_tests as per https://www.jepusto.com/mi-with-clubsandwich/

  robust_across <-
    map(mod$analyses, "robu_main") %>%
  ...
  robust_domain <-
    map(mod$analyses, "robu_interact") %>%
    # add coefficient names as a column
    lapply(function(x) {
      x$coef <- row.names(x)
      x
  ...
  # Get coefficients
  coefs <- map(mod$analyses, "mod") %>% map(coef)

  # Get CR2 variance-covariance matrices
  vcovs <- map(mod$analyses, "mod") %>% map(vcovCR, type = "CR2")

  # Get denominator df from complete-data F-tests
  q <- no_levels - 1
  dfs <-
    map(mod$analyses, "mod") %>%
    map_dfr(Wald_test, vcov = "CR2", constraints = constrain_zero(2:(no_levels)), test = "HTZ") %>%
    mutate(
      eta = df_denom + (q - 1)
    )
  eta <- mean(dfs$eta)
  ...
  # Test of moderator overall
  Cmat <- constrain_zero(2:(no_levels), coefs = res$coefficients)
  ...
  res_tibble <- bind_rows(res_tibble, data.frame(domain = "Overall", Fstat = Fstat, df_num = q, df_denom = eta - q + 1, p_val = p_val))

  # Test of domains
  # Get coefficients
  coefs <- map(mod$analyses, "mod_interact") %>% map(coef)

  # Get CR2 variance-covariance matrices
  vcovs <- map(mod$analyses, "mod_interact") %>% map(vcovCR, type = "CR2")
  ...
  for (i in 1:3) {
    dfs <-
      map(mod$analyses, "mod_interact") %>%
      map_dfr(Wald_test, vcov = "CR2", constraints = constrain_equal(coef_indices <- seq(i, no_levels * 3, by = 3)), test = "HTZ") %>%
      mutate(
        eta = df_denom + (q - 1)
      )
    eta_domain[i] <- mean(dfs$eta)
  }
  ...
  for (i in 1:3) {
    # Test of moderator overall
    Cmat <- constrain_equal(coef_indices <- seq(i, no_levels * 3, by = 3), coefs = res$coefficients)
    Q <- t(Cmat %*% res$coefficients) %*% solve(Cmat %*% res$variance %*% t(Cmat)) %*% (Cmat %*% res$coefficients)
    Fstat <- (eta_domain[i]  - q + 1) / (eta_domain[i]  * q) * as.numeric(Q)
    p_val <- pf(Fstat, df1 = q, df2 = eta_domain[i] - q + 1, lower.tail = FALSE)
    res_tibble <- bind_rows(res_tibble, data.frame(domain = names(res$coefficients)[i], Fstat = Fstat, df_num = q, df_denom = eta_domain[i]  - q + 1, p_val = p_val))
  }

  list(moderator_test = res_tibble, robu_averages = robust_across, robu_domains = robust_domain)
})

single_moderator_tests %>% map_dfr("moderator_test", .id = "moderator") %>% 
  rowwise() %>% 
  mutate(domain = str_remove(domain, "domain"),
...
mod <- with(imp, {V <-  impute_covariance_matrix(vi = var_adj,
  ...
  longevity <- factor(longevity, ordered = FALSE)
  art_focus <- factor(art_focus, ref = FALSE)
...
# TK - loop over overall and domain prefixes (^, Cognitive:, Job-related:), merge all and create table:
map(mod$analyses, "mod_interact") %>% pooled_Wald("^art_focus")
