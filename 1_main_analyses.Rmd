...
groundhog::groundhog.library(c("readxl", "TOSTER", "metafor", "tidyverse", "clubSandwich", "cli", "rsprite2", "esc",
                               "mice", "metacart", "gt", "gtExtras"), date = "2023-07-09")
...
effect_sizes_en <- read_excel("data/english_coding-WIP.xlsx", 
                              ...
                              skip = 2, na = c("#N/A", "NA")) %>% filter(!is.na(ID))
...
rename_vec_en <- c(
  ...
  m_div = "M...32",
  sd_div = "SD...33",
  reliab_div = "Reliability Cronbach's alpha (or note)...34",
  notes_div = "Notes...35",
  name_perf = "Name",
  criterion = "Criterion",
  rater = "Rater",
  items_perf = "Items...39",
  opts_perf = "Options...40",
  m_perf = "M...41",
  sd_perf = "SD...42",
  reliab_perf = "Reliability Cronbach's alpha (or note)...43",
  notes_perf = "Notes...44",
  r = "r",
  d = "d",
  other = "other",
...
dataset <- dataset %>% mutate(n_teams = as.numeric(n_teams), domain = domain %>% str_replace("_", "-") %>% as.factor() %>% relevel(ref = "Demographic"))
...
fig_dat <- dataset %>% 
  ...
  ungroup() %>% 
    ...
    mutate(
      sub_dom = factor(sub_dom, levels = c("Other", sub_dom[sub_dom != "Other"] %>% unique()))
    )
...
data %>%
    ggplot(., aes(x = share, y = sub_dom)) +
  labs(subtitle = subtitle) +
    geom_col(
      aes(fill = if_else(sub_dom == "Other", "grey82", "black")),
      width = 0.8
    )  +
    geom_text(
      aes(
...
dem + cog + job + plot_annotation(caption = glue::glue("NB: Share missing to 100% are measures that combine sub-domains\n({fmt_pct(1 - sum(fig_dat$Demographic$share))} for Demographic, very are in others.)"))
```

### Distribution over sectors & functions

TK: finish with full dataset (as only Top-n can be shown)

```{r}
dataset %>% select(articlestudy, ind_sector, teams_function) %>% 
  distinct() %>% count(ind_sector, sort = TRUE)

dataset %>% select(articlestudy, ind_sector, teams_function) %>% 
  distinct() %>% count(teams_function, sort = TRUE)
```
...
variables <- c("art_focus", "pub_status", "interdep", "complexity", "tmt", "stud_sample",
               "meas_type", "design", "rater", "virtuality", "auth_diff")
...
level_names <- tibble::tribble(
  ~var,           ~level_old,                 ~level_new,                 
   ...
   "pub_status",   "MA Dissertation",          "MA Dissertation",         
...
final_table %>% 
  ...
  fmt_missing(columns = everything(), missing_text = "(missing)") %>% 
  ...
  tab_header("Distribution of moderator variables") %>% 
...
- Group performance (i.e. bonus received; Li, Shemla & Wegge, 2021): .41
Therefore, we assumed a year-on-year correlation of .5 for repeated measures of corporate and team performance.

However, for specific operational measures, the correlation is likely to be substantially different and likely higher. For instance, Pegels et al (2000) report that airline's load factor had a year-on-year correlation of .96. Therefore, we only considered the number of independent observations in these cases.

After that, we calculated the effective sample size using the formula provided by 

$$
N^* = N \times \frac{1 - r}{1 + r}
$$
...
calculate_es <- function(row) {
  ...
  if (!is.na(row$other)) {
    if (str_detect(row$other, "OR")) {
      return(OR_to_r(str_extract(row$other, "\\d.*$") %>% as.numeric(), n = row$n_teams))
    } else {
      message("Challenge in ", row$id, " row: ", row$effect_id)
      return(NA)
      # TK - to be used for esc_B in particular
      # How to assign and return result
      eval(parse(text = row$other))
    }
  }
}
...
sample_reliab <- function(x, items, type, tercile) {
  if (!is.na(x)) return(x)
  if (is.na(items)) {cat(str_sub(type,1,1)); return(NA)}
  if (items == 1) return(1)
  if (type == "div") {
