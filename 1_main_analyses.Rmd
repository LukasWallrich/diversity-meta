...
cur_data$color <- ifelse(cur_data$art_focus == "focal H", 
   ...
   V <- with(cur_data,
    ...
    eggers_label <- glue::glue("Egger's regression test for {domain %>% tolower()} diversity: β = {round(coefs$beta[2], 2)}, *t*({round(coefs$df_Satt[2], 1)}) = {round(coefs$t[2], 2)}, *p* {fmt_p(coefs$p[2])}")
...
apply_3psm <- function(data) {
    ...
    psm_mod <- selmodel(m.rma,
            type = "stepfun",
            steps = 0.025,
            alternative = "two.sided") # Selection for both positive and negative sig values  
