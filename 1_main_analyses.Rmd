...
dataset_aggregate <- escalc(yi = r_adj, vi = var_adj, data = datasets_single)

dataset_aggregate <-   aggregate.escalc(dataset_aggregate, 
                                         cluster = es_aggregate_id, rho = rho, struct = "CS")
```
...
mice.impute.imp_sub_domain <- function(y, ry, x, ...) {
  ...
  for (d in unique_domains) {
    ...
    # Apply polyreg imputation method for the current domain
    args_list <- c(list(y_sub, ry_sub, x_sub), dots_sub)
    imp <- do.call("mice.impute.best_polyreg", args_list)
...
# Derived from mice polyreg function but without noise
# Derived from mice package code, copyright Stef van Buuren
mice.impute.best_polyreg <- function(y, ry, x, wy = NULL, nnet.maxit = 100,
  ...
  max_col_index <- apply(post, 1, which.max)
  colnames(post)[max_col_index]

}

# Derived from mice polr function but without noise
mice.impute.best_polr <- function(y, ry, x, wy = NULL, nnet.maxit = 100,
                             nnet.trace = FALSE, nnet.MaxNWts = 1500,
...
```{r}
# Specify ordered factors
dataset_aggregate$longevity <- factor(dataset_aggregate$longevity, levels = c("hours", "days", "weeks", "months", "years", "stable"), ordered = TRUE)
dataset_aggregate$interdep <- factor(dataset_aggregate$interdep, levels = c("low", "medium", "high"), ordered = TRUE)
dataset_aggregate$complexity <- factor(dataset_aggregate$complexity, levels = c("low", "medium", "high"), ordered = TRUE)
...
mi_vars <- c("art_focus", "pub_status", "design", "setting", "ind_sector", "team_function", "country", "n_teams", "stud_sample", 
             "tmt", "domain", "sub_dom", "meas_type", "rater", "interdep", "complexity", "longevity", "power_distance", "collectivism", 
             "year_merged", "citation_count")

# Get matrices
datasets_sampled_reduced <- datasets_sampled %>% select(articlestudy, domain, r_adj, se, all_of(mi_vars))
imp <- mice(datasets_sampled_reduced, maxit=0, seed = 1526)
...
# Use pmm for factors with many levels
methods[c("ind_sector", "team_function", "country", "sub_dom")] <- "pmm"
methods[!names(methods) %in% mi_vars] <- ""
methods["sub_dom"] <- "imp_sub_domain"
methods[methods == "polr"] <- "best_polr"


# Note that the functions here use column names from predict to determine imputation values
# This can lead to issues with imputed valued that should contain blanks or special characters - in those cases, would need to refine code
imp <- mice(datasets_sampled_reduced, m = 1, seed = 1526, predictorMatrix = preds, 
            method = methods, donors = 1)  

imp2 <- mice(dataset_red, m = 1, seed = 1526, predictorMatrix = preds2, 
            method = methods2, donors = 1)  
