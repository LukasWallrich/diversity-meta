...
groundhog::groundhog.library(c("readxl", "TOSTER", "metafor", "tidyverse", "clubSandwich", "cli", "rsprite2", "esc",
                               "mice", "metacart", "gt"), date = "2023-07-09")
...
dataset <- dataset %>% mutate(n_teams = as.numeric(n_teams), domain = as.factor(domain))
...
meta_summary_intercept <- summary(meta_model_intercept)
...
Average effects significantly differed between diversity domains, `r res[str_detect(meta_summary_intercept, "^QM")]` (though not substantially so? TK - run equivalence test for this?)
After accounting for that, significant amount of heterogeneity remained, `r res[str_detect(meta_summary, "^QE")]`

`r I2s$result[3]$I2/100 %>% fmt_pct()` of the total variance in effect sizes can be attributed to study-level heterogeneity (I<sup>2</sup><sub>Level 3</sub>), while `r I2s$result[2]$I2/100 %>% fmt_pct()` can be attributed to heterogeneity between the effects studied (I<sup>2</sup><sub>Level 2</sub>).
...
# TK - merge with Hofstede data
# TK - retrieve GS citation counts
...
preds[] <- 1  
preds[, !colnames(preds) %in% mi_vars] <- 0
preds[!rownames(preds) %in% mi_vars, ] <- 0
