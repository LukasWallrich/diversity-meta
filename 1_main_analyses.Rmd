...
cont_moderator <- c("year_merged", "collectivism")


single_moderator_tests <- map(cat_moderators, \(moderator) {
  message("Now estimating ", moderator)
no_levels <- dataset[[moderator]] %>% unique() %>% na.omit() %>% length()
            browser()
mod <- with(imp, {
...
Fstat <- (eta - q + 1) / (eta * q) * as.numeric(Q)
p_val <- pf(Fstat, df1 = q, df2 = eta - q + 1, lower.tail = FALSE)
res_tibble <- bind_rows(res_tibble, data.frame(domain = names(res$coefficients)[((no_levels * 1:3)-(3-i))[1]], Fstat = Fstat, df_num = q, df_denom = eta - q + 1, p_val = p_val))
}

list(list(moderator_test = res_tibble, robu_averages = robust_across, robu_domains = robust_domain)) %>% set_names(moderator)
})
```
...
mod <- with(imp, {V <-  impute_covariance_matrix(vi = var_adj,
  ...
  mod_interact <- rma.mv(as.formula(paste0("r_adj ~ ", domain * year_merged + domain * complexity + domain * interdep + domain * collectivism + 
